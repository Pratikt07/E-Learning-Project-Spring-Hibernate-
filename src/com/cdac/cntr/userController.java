...
public class userController {
	...
	public String  regUserForm(User user,HttpServletRequest request, ModelMap map) {
		...
		if(check) {
			request.setAttribute("msg", "You Have Already Registered, Please Log In.");
	...
	public String  logUserForm(User user,HttpServletRequest request, ModelMap map,HttpSession session) {
		boolean check = Userservice.validateUser(user);
		...
		return "loginSuccess";
			...
			map.put("user", new User());
			request.setAttribute("msg", "No Registered Account Found, Please Sign Up.");
			return "sessionInvalidate";
		}
	}
	...
	public String prepUpdateProfile( ModelMap map , HttpSession session,HttpServletRequest request) {
		User u = (User) session.getAttribute("user_s");
	...
	public String updateProfile(User u,ModelMap map ,HttpServletRequest request, HttpSession session) {
			
			User use = (User)session.getAttribute("user_s");
			u.setUserId(use.getUserId());
		boolean check =  Userservice.modifyUser(u);
		
		if(check) {
			request.setAttribute("userPresent", "Mail Id Already Present.");
			return "updateUserProfile";
		}else {
			session.setAttribute("user_s", u);
			map.put("user", u);
			return "userProfile";
		}
